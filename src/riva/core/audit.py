...
def _collect_extra_config_paths() -> list[tuple[str, Path]]:
    ...
    windsurf_user_settings = Path.home() / "Library" / "Application Support" / "Windsurf" / "User" / "settings.json"
...
def _check_api_key_exposure() -> list[AuditResult]:
    ...
    secrets = [
        v
        for v in env_vars
        if "KEY" in v["name"].upper() or "TOKEN" in v["name"].upper() or "SECRET" in v["name"].upper()
    ]
    if secrets:
...
def _check_orphan_processes() -> list[AuditResult]:
    ...
    try:
        ...
        try:
            ...
            if orphans:
                names = ", ".join(f"PID {o['orphan_pid']} ({o.get('orphan_name', '?')})" for o in orphans[:5])
            ...
            if mode & 0o022:
                results.append(
                    AuditResult(
                        ...
                        status="fail",
                        detail=(
                            f"Agent binary '{binary}' is group/world-writable "
                            f"(mode {oct(stat.S_IMODE(mode))}). It could be replaced with malware."
                        ),
                        severity="high",
                        category="permissions",
                    )
            ...
            if launch_type == "unknown":
                ...
                results.append(
                    AuditResult(
                        ...
                        status="warn",
                        detail=(
                            f"Agent '{inst.name}' (PID {inst.pid}) launched by script interpreter '{inst.parent_name}'"
                        ),
                        severity="medium",
                        category="processes",
                    )
